...
if len(sys.argv) > 2:
    ...
    win_id = re.search("window id # (0x[0-9a-f]+),", out).group(1)

# Get info on focused window,
out = subprocess.check_output(["xwininfo", "-id", win_id, "-all"]).decode(
    "ascii", "ignore"
)
geo_str = (
    "Width:",
...
wmctrl(win_id, [["-b", "toggle," + s] for s in state])
wmctrl(win_id, [["-e", "0,%d,%d,-1,-1" % tuple(npos)]])
wmctrl(win_id, [["-b", "toggle," + s] for s in state])
