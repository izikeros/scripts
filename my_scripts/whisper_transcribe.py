...
def transcribe_audio(file_path, prompt, temperature, language, output_file):
    ...
    if not api_endpoint or not api_key:
        raise EnvironmentError("API endpoint or API key is missing in the environment variables.")
    ...
    with open(file_path, 'rb') as audio_file:
        ...
        response = requests.post(api_endpoint, headers=headers, data=json.dumps(data), files=files)
...
if __name__ == "__main__":
    ...
    # Optional arguments
    parser.add_argument("--prompt", help="Optional prompt to provide additional context", default="")
    parser.add_argument("--temperature", type=float, help="Set the decoding temperature (default: 0.7)", default=0.7)
    parser.add_argument("--language", help="Language of the transcription (e.g., 'en', 'fr')", default="en")
    ...
    try:
        transcribe_audio(args.file, args.prompt, args.temperature, args.language, output_file)
