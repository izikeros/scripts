...
DEFAULT_IGNORES = {'.git', '.venv', '__pycache__', '.pytest_cache', '*.pyc', '*.pyo', '*.pyd', '*.egg-info'}
...
def should_ignore(path, ignores):
    ...
    return any(
        any(part.startswith(ignore.strip('*')) or part.endswith(ignore.strip('*')) 
...
def get_modified_files(root_dir, days, sort_by, ignores):
    ...
    if sort_by == 'alpha':
        ...
        return dict(sorted(modified_files.items(), key=lambda x: max(file[1] for file in x[1]), reverse=True))
    
    return modified_files

def main():
    parser = argparse.ArgumentParser(description="List files modified in the last N days grouped by directory.")
    ...
    parser.add_argument("--dirs-only", action="store_true", help="List only directory names")
    parser.add_argument("--top-dirs-only", action="store_true", help="List only top-level directory names")
    parser.add_argument("--sort", choices=['alpha', 'date'], default='alpha', help="Sort output by directory name (alpha) or modification date (date)")
    parser.add_argument("--ignored-add", nargs='*', help="Additional patterns to ignore")
    parser.add_argument("--ignore", nargs='*', help="Patterns to ignore (overrides defaults)")
    ...
    try:
        modified_files = get_modified_files(args.root_dir, args.days, args.sort, ignores)
        
        if args.top_dirs_only:
            top_dirs = set(dir_path.split(os.sep)[0] for dir_path in modified_files.keys())
            ...
            for dir_path, files in modified_files.items():
                if args.dirs_only:
                    ...
                    for filename, mod_time in files:
                        print(f"  {filename} (Modified: {mod_time.strftime('%Y-%m-%d %H:%M:%S')})")
    except Exception as e:
