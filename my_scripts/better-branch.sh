#!/bin/bash
# inspiration from https://youtu.be/aolI_Rz0ZqY?si=3nbEf8m2sKZDJRQU&t=468

current_branch=$(git rev-parse --abbrev-ref HEAD)
branches=$(git for-each-ref --sort=committerdate refs/heads --format='%(refname:short)')
# revert the list of branches
...
# # Ordering branches by last commit time ascending
# branches=$(git for-each-ref --sort=committerdate refs/heads --format='%(refname:short)')
...
# for branch in $branches; do
#   desc=$(git config branch.$branch.description)
#   ahead=$(git rev-list --left-only --count $current...$branch)
#   behind=$(git rev-list --right-only --count $current...$branch)
