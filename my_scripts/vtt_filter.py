...
def get_unique_initials(name: str, used_initials: set) -> str:
    ...
    while unique_initials in used_initials:
        unique_initials = initials + "".join(
            word[1 : i + 1] for word in words if len(word) > i
        )
        i += 1
        ...
        for line in infile:
            ...
            if not line:
                if merge_lines and current_text and not only_mapping:
                    output.write(
                        f"{current_speaker}{html.unescape(current_text.strip())}\n"
                    )
                    current_text = ""
                    current_speaker = ""
                continue
            ...
            if filter_timestamp and re.match(
            ...
            if speaker_match:
                ...
                if shorten_speaker or only_mapping:
                    if speaker_name not in speaker_initials:
                        speaker_initials[speaker_name] = get_unique_initials(
                            speaker_name, used_initials
                        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--keep-speaker", action="store_true", help="Keep speaker names"
    )
    parser.add_argument("--keep-timestamp", action="store_true", help="Keep timestamps")
    parser.add_argument(
        "--merge-lines", action="store_true", help="Merge lines from the same speaker"
    )
    parser.add_argument(
        "--shorten-speaker",
        action="store_true",
        help="Shorten speaker names to unique initials",
    )
    parser.add_argument(
        "--only-mapping",
        action="store_true",
        help="Display only the speaker mapping (implies --shorten-speaker)",
    )
