...
def main():
    parser = argparse.ArgumentParser(description="Generate a CSV list of event names and dates.")
    parser.add_argument("root_dir", help="Parent directory containing year and event folders")
    parser.add_argument("-o", "--output", default="photo_events.csv", help="Output CSV file name")
    parser.add_argument("-s", "--summary", action="store_true", help="Display summary")
    parser.add_argument("-q", "--quiet", action="store_true", help="Quiet mode, suppress progress and summary messages")
    args = parser.parse_args()

    if os.path.exists(args.output):
        if not args.quiet:
            choice = input(f"Output file '{args.output}' already exists. Overwrite? (y/n): ")
            if choice.lower() != "y":
                print("Exiting.")
                return
    ...
    if args.summary:
        ...
        year_count = sum(1 for entry in os.listdir(args.root_dir) if len(entry) == 4 and entry.isdigit())
