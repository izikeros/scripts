        ...
        try:
            ...
            if "database is locked" in str(e):
                if attempt < max_attempts - 1:
                    print(f"Database is locked. Retrying in {delay} seconds... (Attempt {attempt + 1}/{max_attempts})")
                    time.sleep(delay)
                else:
                    raise Exception("Failed to access the database after multiple attempts. Please ensure Brave browser is closed and try again.") from e
            else:
                raise
...
try:
    ...
    print("3. If you're using any sync or backup software, temporarily disable it and retry.")
